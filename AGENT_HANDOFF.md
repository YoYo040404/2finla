...
| Commit | Description |
|---|---|
| `c1cb8d9` | feat: upgrade product case visuals and whatsapp behavior — Premium Jewelry Case: every secondary tile has a visual; new `components/home/CategorySilhouette.tsx` fine-line case-marker SVGs for RINGS/BRACELETS/EARRINGS (gold/ice ink, non-photographic, NOT fake inventory); sharper anchor sub-copy; text-first WhatsApp exit under the case (`WA_CASE`, "Don't see it in the case? TEXT 2T — ASK WHAT'S IN →"). WhatsApp behavior: StickyConversionBar shows only after sentinel scrolls ABOVE viewport (`boundingClientRect.top < 0`) and sets `body.sticky-bar-active` to hide the floating WhatsApp button — the two never stack; MobileCtaBar stays the mobile path. Build + Playwright QA clean |
| `0302a6a` | feat: sharpen hero bang and subhead — hero subhead readability (white first line ~500 wt + silver second line + text-shadow over video): `Grillz. Watches. Chains. Pendants. Custom pieces.` / `Real Pittsburgh jeweler. Text 2T before you buy.` `BANG.` is the approved form — **never `BANG!`**; fake CSS/SVG iced-pendant BANG rejected (looked AI); real BANG pendant = future asset/render project |
...
| `0b1a47b` | copy: sharpen site CTAs and promo rhythm — B2 copy pass (S1–S4). Collections hub card CTAs → `SHOP {LABEL} →` / `ASK ABOUT GRILLZ →` / `ASK ABOUT WATCHES →` / `START CUSTOM →` (real `→`, was ascii label). Homepage promo de-dup (removed second "ASK WHAT'S RUNNING" + "PROMO TERMS" from inline ticker → `THIS WEEK AT 2T · CHAINS · PENDANTS · GRILLZ · CUSTOM · TEXT 2T →`; global PromoBar unchanged). PICK THE PIECE eyebrow `WHAT 2T MAKES` → `SHOP THE CASE`. Mobile menu WA `HIT US UP →` → `TEXT 2T →`. Copy-string only; build + Playwright QA clean; no layout/CSS/asset/claim/Shopify changes |
| `20e30e4` | copy: tighten social teaser and audience doctrine — SocialTeaser tightened; audience doctrine sharpened in `.claude/skills/2t-copy-chief/SKILL.md` |
| `c4e6eed` | docs: add 2t copy chief skill — `.claude/skills/2t-copy-chief/SKILL.md` added; governing copy + audience system, pairs with `.interface-design/system.md` (UI) and `CLAIM_SAFETY.md` (claims) |
| `a113a70` | feat: simplify homepage proof rhythm — removed ProofMarquee from homepage (trust facts already in ProofContactZone; cut duplicate top marquee + double-ticker noise). ShineRail / Most Asked About kept. Final rhythm: Hero → PICK THE PIECE → ShineDirectionBand → Promo → Custom lane → ShineRail → ProofContactZone → SocialTeaser → FinalCTABar. `app/page.tsx` only; components/CSS/data intact (ProofMarquee unmounted, not deleted). Live-approved. No copy/claim/asset/Shopify changes |
| `fbd203c` | feat: relocate ShineRail below custom lane — moved ShineRail / Most Asked About from upper page to after the Custom lane, before ProofContactZone; cut back-to-back category repetition. JSX move only |
...
**Current focus:** Latest work is the Hero + Product Case + WhatsApp series (`d16f7ce` → `0302a6a` → `c1cb8d9`). Approved direction = loud, flashy, icy, product-first, street-luxury — but **controlled (= precise, NOT quiet/generic)**, premium, readable, claim-safe. Visuals create desire first; copy then drives Text 2T. Hero `BANG.` is the approved form (never `BANG!`); the fake CSS/SVG iced-pendant BANG was rejected for looking AI — a real BANG pendant is a future asset/render project. Product Case (PICK THE PIECE) = Premium Jewelry Case, visual-first: all 7 categories carry a visual, no text-only tiles, RINGS/BRACELETS/EARRINGS use `CategorySilhouette` case markers (not fake inventory) until real macros arrive. WhatsApp floating button and StickyConversionBar no longer stack. **Next recommended phase: Custom Order (`/custom`) page audit / strategy pass — art direction, copy, conversion path, claim-safety, visual hierarchy first; code only after approval.** Preceded by the B2 copy-sharpening pass (`0b1a47b`: collections hub CTAs, homepage promo de-dup, `SHOP THE CASE` eyebrow, mobile-menu `TEXT 2T →`) on top of SocialTeaser tightening + audience doctrine (`20e30e4`) and the 2T Copy Chief skill (`c4e6eed`). All copy-string only, build + Playwright QA clean. Preceded by: Homepage proof-rhythm simplification completed, live-approved by Yaniv, and pushed (`a113a70` + `fbd203c`). ProofMarquee removed from homepage; ShineRail / Most Asked About kept and relocated below the Custom lane (before ProofContactZone). Final rhythm (9 sections): Hero → PICK THE PIECE → ShineDirectionBand → Promo strip → Custom lane → ShineRail / Most Asked About → ProofContactZone → SocialTeaser → FinalCTABar. Promo strip is now the only ticker. Reason: cut repeated trust/ticker noise, keep Most Asked About + shop energy, shorter/cleaner page. `app/page.tsx` only; no copy/claim/asset/Shopify changes. Next phase: SocialTeaser tightening/upgrade is a possible separate future pass — plan and approve before any implementation. Preceded by Trust + Text-Speed + Custom Hero Pass A (`98c56ff`), Homepage De-Clutter Pass 1 (`c24fd27`), Visual Pass 1B (`a785b85`), and Midnight Flash visual polish (`e1caaba`). Pass 5C Copy/CRO (`5da57da`) and Interface Design system at `.interface-design/system.md` (`553834b`) remain in place — the latter is the governing file for all future UI/visual work. Current next phase: evaluate homepage after declutter before adding more motion or sections, then continue visual/UI pass planning. No further implementation beyond what is committed. Candidate work: P0 eyebrow standardization + gold token consolidation + card padding system + MobileCtaBar route audit; P1 category cross-sell rail + About page proof-ledger rebuild; P2 surface token system. Top motion add-on candidates: Hero Video Vignette, BANG. Sparkle Burst on Entry, Card Border Edge Glint. Do not implement any visual pass without explicit approval.
...
| PICK THE PIECE gateway | ✅ Premium Jewelry Case, visual-first (`c1cb8d9`). Anchor row (GRILLZ dominant) + TileGlareController 3D tilt/glare desktop. Secondary row = case shelf, every tile has a visual: PENDANTS demo macro; RINGS/BRACELETS/EARRINGS = `CategorySilhouette` fine-line case markers (gold/ice, non-photographic, NOT fake inventory). No text-only tiles. Text-first `WA_CASE` exit under the case. Material/stone row + discovery strip removed in De-Clutter Pass 1 (`c24fd27`) |
...
| StickyConversionBar | ✅ Pass 4B + `c1cb8d9` — desktop/tablet WA CTA, z-index 30, hidden on mobile, sessionStorage dismiss. Now shows only after the sentinel scrolls ABOVE the viewport (`boundingClientRect.top < 0`) and sets `body.sticky-bar-active` to hide the floating WA button — the two never stack. |
...
**Connected MCPs (as of 2026-06-21):** Playwright, Firecrawl, Magic 21st.dev, Next DevTools, Chrome DevTools, Context7, Magic UI. **Forbidden for 2T unless explicitly approved:** Shopify (paused), Gmail, Calendar, Drive, Spotify.
